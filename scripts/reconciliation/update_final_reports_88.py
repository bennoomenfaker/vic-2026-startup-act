...
for p in mds:
    ...
    text = text.replace('3 555 lignes/candidatures documentaires', '3 555 lignes PDF documentaires + 3 ajournés hors PDF = 3 558 candidatures corrigées')
    text = text.replace('3 555 lignes/candidatures', '3 555 lignes PDF + 3 ajournés hors PDF = 3 558 candidatures corrigées')
    text = text.replace('3 555 lignes documentaires retenues pour l’étude', '3 555 lignes PDF + 3 ajournés hors PDF = 3 558 candidatures corrigées')
    text = text.replace('Candidatures selon réexamen PDF | 3 555', 'Candidatures corrigées de l’étude | 3 558')
    text = text.replace('Candidatures corrigées PDF | 3 555', 'Candidatures corrigées de l’étude | 3 558')
    text = text.replace('Candidatures selon réexamen PDF | **3 555**', 'Candidatures corrigées de l’étude | **3 558**')
    text = text.replace('Candidatures selon le réexamen PDF | 3 555', 'Candidatures corrigées de l’étude | 3 558')
...
for p in [D / 'etude_quantitative' / 'academic_metrics.json', D / 'rapport_academique' / 'academic_metrics.json']:
    ...
    d.update({
        ...
        'lignes_pdf_detaillees': 3555,
        'candidatures_reexamen_pdf': 3555,
        'candidatures_corrigees': 3558,
        'ajournes_hors_pdf': 3,
