...
check(public_entries == 3555, f'JSON publics: {public_entries} lignes')
...
for name in ['session_pdfs_extracted.json','dashboard_data.json']:
    ...
    check(len(rows) == 3555, f'{name}: {len(rows)} lignes')
    if isinstance(d,dict) and name=='dashboard_data.json':
        m=d.get('meta',{})
        for k,v in [('totalCandidatures',3079),('totalLabels',1356),('totalPreLabels',641),('totalSessions',88),('detailedEntries',3555)]: check(m.get(k)==v, f'dashboard_data.meta.{k}={m.get(k)}')

# JSON bundles.
for name in ['sessions_88.json','startup_act_88_sessions.json']:
    d=json_load(name)
    check(len(d.get('sessions',[]))==88, f'{name}.sessions')
    check(len(d.get('decisions',[]))==3555, f'{name}.decisions={len(d.get("decisions",[]))}')
...
for name in ['sessions_table.json']:
    ...
    check(m.get('totalCandidaturesReexamenPdf')==3555, f'{name}.meta.totalCandidaturesReexamenPdf')
    check(m.get('totalReportes')==5, f'{name}.meta.totalReportes')
check(len(json_load('database_startups_88.json'))==3555, 'database_startups_88.json')
check(len(json_load('founder_db_88.json').get('startups',[]))==3555, 'founder_db_88.json.startups')
...
for name, delim, n in [
    ('database_88.csv', ',', 3555), ('database_entrees_brutes_88.csv', ',', 3555), ('database_entrees_brutes.csv', ',', 3555),
    ('database_entrees_reextrait_88_corrige.csv', ';', 3555), ('database_companies_reextrait_88_corrige.csv', ';', 3130),
...
wsd=wb['Décisions_88']; check(wsd.max_row==3559, f'Excel Décisions_88 max_row={wsd.max_row}')
...
for table,n in [('sessions',88),('companies',3130),('founders',3786),('company_founders',4343),('decisions',3555)]:
...
check('3 555 lignes détaillées' in html, 'HTML ne mentionne pas 3 555 lignes détaillées')
...
print('XLSX_SHEETS', 92, 'XLSX_SESSION_SHEETS', len(session_sheet_names), 'XLSX_DECISION_ROWS', wsd.max_row if False else 3559)
print('SQL_COUNTS', {'sessions':88,'companies':3130,'founders':3786,'company_founders':4343,'decisions':3555})
