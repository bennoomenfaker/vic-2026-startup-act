...
const REPORT_SESSIONS = new Set(["S11", "S12", "S28", "S67"]);
...
function QualityView({ qualityKpis }: { qualityKpis: readonly KpiRow[] }) {
  return <section className="content-stack"><div className="view-intro"><div><div className="section-kicker">PROVENANCE & QUALITÉ</div><p>Une vue de contrôle pour distinguer les indicateurs calculés, externes et à collecter.</p></div><div className="quality-summary"><span><i className="quality-dot ok" /> calculés</span><span><i className="quality-dot warn" /> externes</span><span><i className="quality-dot miss" /> à collecter</span></div></div><div className="quality-grid">{qualityKpis.map((kpi) => <article className="quality-card" key={kpi.id}><div className="kpi-id">{kpi.id}</div><h2>{kpi.nom}</h2><p>{kpi.valeur}</p><div className={`catalogue-badge ${kpi.statut}`}>{kpi.statut === "warn" ? "Donnée externe" : "À collecter"}</div></article>)}</div><div className="panel provenance-panel"><div className="panel-kicker">MÉTHODE</div><h2>Les chiffres ne sont pas tous du même type</h2><p>Les compteurs Labels et Prélabels sont issus des 85 sessions. Les KPI géographiques, économiques ou de propriété intellectuelle peuvent dépendre de rapports externes. Le statut reste affiché pour éviter de confondre une donnée calculée et une donnée à actualiser.</p></div><div className="panel provenance-panel"><div className="panel-kicker">CORRECTION S62 · 05/2024</div><h2>39 candidatures, 46 enregistrements documentaires</h2><p>Le PDF officiel distingue 39 candidatures du bloc principal, 5 conversions historiques Prélabel → Label et 2 retraits. Les 46 enregistrements détaillés ne doivent donc pas être transformés mécaniquement en candidatures par la formule « entries − reportés ». À l’échelle du corpus, le dashboard affiche 3 394 lignes détaillées et 3 383 candidatures dans la série corrigée après exclusion des quatre reports confirmés. Les quatre reports confirmés sont signalés en couleur dans le tableau : Tunisia Biotech (02/2020), Campus Numérique des métiers (03/2020), TN Smartbot (07/2021) et RYBSEN (10/2024, reporté à novembre 2024). L’ancien total de 54 lignes Reporté est contaminé par 45 faux positifs S17–S20 ; il doit être séparé des quatre reports confirmés.</p></div></section>;
}
