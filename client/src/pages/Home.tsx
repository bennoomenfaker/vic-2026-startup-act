...
export default function Home() {
  ...
  return (
    <div className="app-shell">
      ...
      {selectedKpi && <div className="modal-backdrop" onClick={() => setSelectedKpi(null)}><div className="kpi-modal" onClick={(e) => e.stopPropagation()}><button className="modal-close" onClick={() => setSelectedKpi(null)} aria-label="Fermer">×</button><div className="kpi-modal-id">{selectedKpi.id} · {selectedKpi.page}</div><h2>{selectedKpi.nom}</h2><div className="modal-value">{selectedKpi.valeur}</div><p>Ce KPI est repris du catalogue du dépôt ou calculé de manière dérivée à partir des données des 88 sessions. Les chiffres externes ou non calculables restent marqués.</p><div className={`catalogue-badge ${selectedKpi.statut}`}>{selectedKpi.statut === "ok" ? "Calculé" : selectedKpi.statut === "warn" ? "Donnée externe" : "À collecter"}</div></div></div>}
    </div>
  );
}
